...
if (!empty($_GET["id"]) && empty($_POST['id'])) {
    ...
    try {
        ...
        // データを更新
        $sql = "UPDATE message SET view_name='" . $view_name . "', message='" . $message . "', date_time='" . $date_time . "' WHERE id=" . $id;
        $data = $dbh->query($sql);
    ...
    <form method="post" action="<?php print($_SERVER['PHP_SELF']) ?>">
        <?php foreach ($dataArr as $data) : ?>
            <div>
                ...
                <input type="text" id="view_name" name="view_name" value="<?php echo $data["view_name"]; ?>">
            </div>
            <div>
                <label for="message">ひと言メッセージ</label>
                <textarea id="message" name="message"><?php echo nl2br($data["message"]); ?></textarea>
            </div>
        <?php endforeach; ?>
